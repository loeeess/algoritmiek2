// Hoofdprogramma voor test van oplossing voor tweede programmeeropdracht
// Algoritmiek, voorjaar 2022: Klaverjassen.
//
...
// instantie s1.
void VerwerkKeuze123 (Schema *s1, Schema2 *s2, int keuze)
...
void menuVoorInstantie (Schema *s1, Schema2 *s2)
{ int keuze,
  ...
  while (keuze!=5)
  {
    ...
    { case 1:
      case 2:
      case 3: VerwerkKeuze123 (s1, s2, keuze);
              break;
      case 4: cout << endl;
...
{ Schema *s1;  // pointer, om makkeijk nieuwe objecten te kunnen maken
  ...
  Schema2 *s2; // pointer, om makkeijk nieuwe objecten te kunnen maken
               // en weer weg te gooien
  int keuze,
      nrSpelers;
  ...
  do
  {
    ...
    { case 1: cout << endl;
              ...
              s2 = new Schema2 (nrSpelers);
              menuVoorInstantie (s1, s2);
              delete s1;  // netjes opruimen
              delete s2;  // netjes opruimen
              break;
      case 2: s1 = new Schema ();
              ...
              { if (s2 -> leesInDeelschema (invoernaam))
                  menuVoorInstantie (s1, s2);
                else
                { cout << endl;
                  cout << "s1 kan tekstbestand wel lezen, maar s2 niet."
                       << endl;
                }
              }
              delete s1;  // netjes opruimen
